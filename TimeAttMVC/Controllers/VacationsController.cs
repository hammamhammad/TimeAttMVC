...
namespace TimeAttMVC.Controllers
{
    ...
    [CustomAuthorizeMVC(GroupNo = "6", PermissionName = "vac-view")]
    public class VacationsController : BaseController
    {
        ...
        private ReportDocument GetReportDocument(FormParameters Para)
        {
            ...
            var RESTServiceForVacations = new RESTService<ResponseResult<IEnumerable<VacationsInfo>>>(WebApiUrl + "Vacations/GetAll");
            var RESTServiceForCompany = new RESTService<ResponseResult<IEnumerable<CompanyInfo>>>(WebApiUrl + "Company/GetWithLogo");
            var dataForVacations = RESTServiceForVacations.Post(Para);
            var dataForCompany = RESTServiceForCompany.Get();
            ReportDocument rd = new ReportDocument();
            if (!string.IsNullOrEmpty(Para.GroupBy) && Para.GroupBy == "bybranch")
            {
                rd.Load(Server.MapPath("~/Reports/" + (cult == "ar" ? "AR" : "EN") + "/VacationsReportByBranch.rpt"));
            }
            else if (!string.IsNullOrEmpty(Para.GroupBy) && Para.GroupBy == "bysection")
            {
                rd.Load(Server.MapPath("~/Reports/" + (cult == "ar" ? "AR" : "EN") + "/VacationsReportBySection.rpt"));
            }
            else
            {
                rd.Load(Server.MapPath("~/Reports/" + (cult == "ar" ? "AR" : "EN") + "/VacationsReport.rpt"));
            }

            rd.Database.Tables["spGetOrganization;1"].SetDataSource(dataForCompany.Result.ToDataSet().Tables[0]);
            rd.Database.Tables["spsearchVacations;1"].SetDataSource(dataForVacations.Result.ToDataSet().Tables[0]);
        ...
        [CustomAuthorizeMVC(GroupNo = "8", PermissionName = "rep-vac")]
        ...
        public ActionResult PrintReport(string fileType, [System.Web.Http.FromBody] FormParameters Para)
        {
            try
            {
                ...
                return ReturnReport(fileType, rd, "Vacations_" + Para.FromDate.GetDateFromNumber().ToString("yyyyMMdd", Culture) + "_" + Para.ToDate.GetDateFromNumber().ToString("yyyyMMdd", Culture));
            }
            catch (Exception ex)
            {
