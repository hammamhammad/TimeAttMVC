...
export class MytasksComponent implements OnInit , AfterViewInit{
  ...
  GetTaskStatus() {
    this.spinner.show();
     ...
     this.api.getRequest(AppSettings.WebApiUrl + 'workflow/GetWorkFlowStatus')  .finally(() => {
          this.spinner.hide();
  ...
  GetMyAllTasks() {
    this.spinner.show();
     ...
     this.api.getRequest(AppSettings.WebApiUrl + 'workflow/GetMyAllTasks/' + this.taskStatus) .finally(() => {
          this.spinner.hide();
      }).subscribe((res: ResponseResult) => {
      if (res.Status) {
