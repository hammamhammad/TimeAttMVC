...
export class MyReportComponent implements OnInit, AfterViewInit {
  ...
  getMyAtt() {
    ...
    this.api.post(AppSettings.WebApiUrl + 'SelfServices/TimeSheet/GetMonthDetails', this.MyTimeForm, true).finally(() => {
      this.spinner.hide();
  ...
  UserSelectiontoviewDailyDetailsEMP(empid, mdate) {
    this.spinner.show();



     this.api.getRequestAS(AppSettings.WebApiUrl + "SelfServices/TimeSheet/GetTransDetailsForEmployee/" + empid + "/" + mdate).finally(() => {
      this.spinner.hide();
  }).subscribe(res => {
      if (res.Status == 1) {
