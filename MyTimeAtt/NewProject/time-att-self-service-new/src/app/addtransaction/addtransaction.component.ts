...
export class AddtransactionComponent implements OnInit {
  ...
  constructor(private api: APIRestFulService, private router: Router,
    ...
    private _notifcation: Notifications, private translate: TranslateService, private http: HttpClient) { }
  ...
  getShiftInfo(): void {
    ...
    var date = new Date(this.transForm.transDate).ToOADate()
    console.log(date);
    console.log(this.UserInfo?.UserEmpID!)
    this.api.getRequestAS(AppSettings.WebApiUrl + 'SelfServices/Excuse/GetShiftInfo/'+ date +'/' + this.UserInfo?.UserEmpID!).pipe(
  ...
  canAddExc() {
    ...
    this.spinner.show();
    this.api.getRequestAS(AppSettings.WebApiUrl + 'SelfServices/Trans/CanAddTransaction/' + this.transForm.DateNo + '/' + this.transForm.m_time + '/' + this.transForm.m_transtype) .pipe(
      finalize(() => this.spinner.hide())
 ...
 this.api.postFormData(apiUrl1, formData)
  ...
  ShowExecuseErrormsg(msg: string | Document | Element | Comment | DocumentFragment | ((this: HTMLElement, index: number, oldhtml: string) => string | JQuery.Node)) {
