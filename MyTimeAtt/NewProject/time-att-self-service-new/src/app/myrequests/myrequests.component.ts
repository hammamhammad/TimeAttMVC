...
export class MyrequestsComponent implements OnInit, AfterViewInit {
  ...
  @ViewChild('MyRequestTable')
  MyRequestTable!: DataTableDirective;
  ...
  GetTaskStatus() {
    this.spinner.show();
     ...
     this.api.getRequestAS(AppSettings.WebApiUrl + 'workflow/GetTaskStatus').pipe(
  ...
  GetMyRequests() {
    ...
    this.api.getRequestAS(AppSettings.WebApiUrl + 'workflow/GetMyAllRequest/' + this.taskStatus).pipe(
      finalize(() => this.spinner.hide())
  ).subscribe((res: ResponseResult) => {
      if (res.Status) {
