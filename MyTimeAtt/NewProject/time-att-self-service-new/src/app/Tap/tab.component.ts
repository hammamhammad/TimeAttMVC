import { Component, Directive, Input, ContentChildren, QueryList, TemplateRef, ContentChild, AfterContentInit, Output, EventEmitter, OnDestroy, AfterContentChecked, Optional } from '@angular/core';
    ...
    template: `
      <div class="tab-set">
        <ul class="nav nav-tabs">
          <li class="nav-item" *ngFor="let tab of tabs" [class.active]="tab.id === activeId" [hidden]="!tab.isVisable">
            <a class="nav-link" (click)="select(tab.id)" [class.active]="tab.id === activeId" [class.disabled]="tab.disabled">
              {{ tab.title }}
              <ng-container *ngTemplateOutlet="tab.titleTpl!.templateRef"></ng-container>
            </a>
          </li>
        </ul>
        <div class="tab-content">
          <ng-container *ngFor="let tab of tabs">
            <div class="tab-pane" [hidden]="tab.id !== activeId" [attr.id]="tab.id">
              <ng-container *ngTemplateOutlet="tab.contentTpl!.templateRef"></ng-container>
            </div>
          </ng-container>
        </div>
      </div>
...
export class NgbTabset implements AfterContentChecked, OnDestroy {
    ...
    private _getTabById(id: string): NgbTab |null {
        let tabsWithId: NgbTab[] = this.tabs.filter(tab => tab.id === id);
        return tabsWithId.length ? tabsWithId[0] : null;
    }
    private _getTabByIndex(index: number): NgbTab |null{
...
export class TabAutofocusDirective {
    constructor(@Optional() private tab: NgbTabset) {
        if (tab) {
