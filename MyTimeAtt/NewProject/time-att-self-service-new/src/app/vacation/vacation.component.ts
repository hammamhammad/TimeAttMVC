...
export class VacationComponent implements OnInit, AfterViewInit {
  ...
  constructor(private api: APIRestFulService, private router: Router,
    private authService: AuthService, private _notifcation: Notifications, private translate: TranslateService, private http: HttpClient, private spinner: LodingSpinnerService) { }
  ...
  ShowErrormsg(msg: string | Element | Comment | Document | DocumentFragment | ((this: HTMLElement, index: number, oldhtml: string) => string | JQuery.Node)) {
  ...
  GetVacationTypes() {
    ...
    this.api.getRequestAS(AppSettings.WebApiUrl + 'SelfServices/Vacations/Types/GetAll').pipe(
      finalize(() => this.spinner.hide())
      ).subscribe((res: { Status: number; Result: any[]; }) => {
      if (res.Status == 1) {
        this.vacationTypes = res.Result.filter((e: { istrip: boolean; }) => e.istrip == true);
      }
  ...
  canAddVac() {
    ...
    this.spinner.show();
     ...
     this.api.post(AppSettings.WebApiUrl + 'SelfServices/Vacations/CanAdd', this.vacationForm, true).pipe(
      finalize(() => this.spinner.hide())
