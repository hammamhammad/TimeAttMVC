...
export class ViolationComponent implements OnInit, AfterViewInit {
  ...
  FillViolationTables() {
    ...
    this.api.getRequestAS(AppSettings.WebApiUrl + 'workflow/GetViolationTaskDetails/' + this.taskID).pipe(
      finalize(() => this.spinner.hide())
    ).subscribe((res: { Status: number; Result: { CompanyName: any; PeriodStart: any; PeriodEnd: any; Absences: any; Lateness: any; Violations: any; }; Msg: any; }) => {
      if (res.Status == 1) {
