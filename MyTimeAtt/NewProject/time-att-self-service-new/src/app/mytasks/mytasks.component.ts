...
export class MytasksComponent implements OnInit , AfterViewInit{
  ...
  @ViewChild('MyTasksTable') MyTasksTable!: DataTableDirective;
  dtOptionsMyTasksTable: DataTables.Settings = {};
  dtTriggerMyTasksTable: Subject<any> = new Subject();
  isRenderMyTasksTable = false;
  ...
  GetTaskStatus() {
    this.spinner.show();
     ...
     this.api.getRequestAS(AppSettings.WebApiUrl + 'workflow/GetWorkFlowStatus').pipe(
  ...
  GetMyAllTasks() {
    this.spinner.show();
     ...
     this.api.getRequestAS(AppSettings.WebApiUrl + 'workflow/GetMyAllTasks/' + this.taskStatus) .pipe(
      finalize(() => this.spinner.hide())
      ).subscribe((res: ResponseResult) => {
        console.log("tttttttttttttttttt22222222222222222")
        console.log(res)
   if (res.Status) {
        this.myRequestsList = res.Result;
        if (!this.isRenderMyTasksTable) {

          this.dtTriggerMyTasksTable.next("");
          this.isRenderMyTasksTable = true;
  
        } else {
          this.MyTasksTable.dtInstance!.then((dtInstance: DataTables.Api) => {
